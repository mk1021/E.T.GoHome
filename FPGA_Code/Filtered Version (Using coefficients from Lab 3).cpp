//Filtered Version (Using coefficients from Lab 3)
...
alt_32 average(alt_32 *m, float coefficients[]){
alt_32 sum = 0;
for(int i=0; i<FILTER_ORDER; i++){
sum += (float)m[i]* coefficients[i];
}
return (alt_32)sum;
}
...
int main() {
    ...
    while (1) {
        ...
        x_mem[i] = x_mem[i-1]; }
        x_mem[0] = x_read;
        x_output = average(x_mem, coefficients);
 
        alt_up_accelerometer_spi_read_y_axis(acc_dev, & y_read);
        for(int i=4; i>0; i--){
                y_mem[i] = y_mem[i-1]; }
                y_mem[0] = y_read;
                y_output = average(y_mem, coefficients);
