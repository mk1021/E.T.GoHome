...
def main():
    ...
    while not done:
        ...
        if player.rect.x > 801:
            ...
            if player_num == 1:
                if current_room_no == 1:
                    print('e')
                    client_socket.send(("Room1/player1finished").encode())
                    print('f')
                    status_room1 = client_socket.recv(1024).decode()

                    if status_room1.startswith("Room1_finished"):
                        if playerwaited:
                            if room1_b == 0:
                                roomwait_stop = elapsed_time
                                pause = roomwait_stop - roomwait_start
                                playerwaited = False
                                room1_b = 1
                        current_room_no = 2
                        current_room = rooms[current_room_no]
            ...
            if player_num == 2: 
                if current_room_no == 1:
                    client_socket.send(("Room1/player2finished").encode())
                    status_room1 = client_socket.recv(1024).decode()
        ...
        #PLAYER 2====================================
         ...
         elapsed_time = int(time() - start_time) - pause
